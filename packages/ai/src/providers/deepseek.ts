...
const log = logger.child({ feature: 'ai', component: 'DeepSeekProvider' })

/**
 * DeepSeek implementation of both provider slots.
 *
 * DeepSeek's Chat Completions API is OpenAI-compatible — this mirrors
 * OpenAIProvider's/MistralProvider's request shape against
 * `https://api.deepseek.com/v1` instead of duplicating a different
 * transport, with one confirmed difference: DeepSeek rejects strict
 * `response_format: json_schema` (HTTP 400), so `performChat` sends
 * `json_object` mode instead — see its own comment for what that changes.
 */
export class DeepSeekProvider implements AIProvider, DictionaryProvider {
  ...
  constructor(config: DeepSeekProviderConfig) {
    ...
    // needed 30s instead of comfortably clearing 15s; a full word-package generation timed out at
    // the standard 60_000ms default) — not a one-off hang, so this is a longer baseline rather
    // than raising every provider's timeout to match.
  ...
  private async chat(
    ...
    log.debug('ai.provider_request_started', { message: 'DeepSeek chat completion request started', metadata: meta })

    try {
      const completion = await this.performChat(messages, schemaName, jsonSchema, startedAt)
      log.info('ai.provider_request_completed', {
        message: 'DeepSeek chat completion request succeeded',
        result: 'success',
      ...
      log.error('ai.provider_request_failed', error, {
        message: 'DeepSeek chat completion request failed',
  ...
  private async performChat(
    ...
    // DeepSeek's Chat Completions API rejects strict response_format: json_schema outright (HTTP
    // 400 "This response_format type is unavailable now", confirmed against a live key) — only
    // the looser `json_object` mode is currently supported, which guarantees syntactically valid
    // JSON but doesn't enforce a schema shape server-side. That's a bigger loss than it first
    // looks: every provider's shared prompt text (prompts/templates.ts) was written assuming
    // strict mode fills in the gaps — it describes enum fields loosely (or, for a while, not at
    // all — see wordPackage v7's doc comment) because OpenAI/Mistral/Gemini/Claude force a valid
    // enum value regardless. Patching each gap in the shared prompt text one at a time doesn't
    // scale (confirmed live: even after the cefrLevel fix, a real generation still came back with
    // 16 separate validation issues) — so instead, the same jsonSchema the caller already builds
    // (previously unused here) is appended to the prompt as text, giving the model the same
    // structural contract strict mode would have enforced, just delivered a different way.
    // json_object mode also requires the word "json" to appear somewhere in the conversation,
    // satisfied by the same appended text.
    ...
    if (!response.ok) {
      ...
      throw new AIProviderError(
        `DeepSeek returned ${response.status}: ${body.slice(0, 500)}`,
        this.name,
    ...
    if (typeof content !== 'string' || content === '') {
      throw new AIProviderError('DeepSeek response contained no content', this.name, true)
    }
