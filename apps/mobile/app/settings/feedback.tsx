...
/**
 * "Send Feedback" — UI shell for the flow designed in LingoraDocs/10_feedback_to_github_issue.md.
 * Deliberately frontend-only for now: creating a GitHub issue needs a repo-write token that can
 * never ship inside the app (see that doc's §2), so there is no real submission path yet — Submit
 * just confirms locally.
 *
 ...
 *
 * Written to be portable to another Expo app (Shelfie and others) with minimal changes — the form
 * itself (category/title/message/diagnostics/contact fields, validation, the help accordion) has
 * no Lingora-specific data model. To reuse it elsewhere:
 *   1. Copy this file and swap the `components/ui` import (`Button`/`Card`/`Chip`/`IconButton`)
 *      and the `useColors`/`useThemedStyles` import for the target app's own UI kit/theme hook —
 *      or drop theming and hardcode colors if the target app has none yet.
 *   2. Replace the `useServices().tier` line and its one use in the diagnostics preview with
 *      whatever build-relevant, non-identifying signal the target app wants to include (or drop
 *      the tier segment entirely) — everything else in the diagnostics block (app version via
 *      `expo-constants`, `Platform.OS`) is already generic.
 *   3. Update the doc-comment/log-event feature name, the GitHub repo mentioned in HELP_SECTIONS'
 *      "public issue" copy, and swap the `@lingora/observability` logger import for the target
 *      app's own logger (or drop the one `log.info` call — it's a diagnostic, not load-bearing).
 * Everything else — the three-category chip picker, the field layout, the diagnostics
 * toggle/preview, and the whole help-accordion integration — needs no changes.
 */
