...
**Phase status (keep this current):** Phase 1 ✅ · Phase 2 (database/search/morphology) ✅ · Phase 3 (AI) ✅ core engine in `packages/ai` (OpenAI, Mistral, Gemini, Anthropic/Claude, and DeepL providers — the first four implement both `AIProvider` and `DictionaryProvider`, DeepL implements `DictionaryProvider` only; Google Translate free-tier dictionary adapter; validation/repair, prompt versioning, cache, persistence pipeline); Wiktionary adapter pending · Phase 4 (DB+AI wiring) ✅ search, word detail, home, decks, deck detail, mining and settings (SecureStore keys per provider, validate-key + device-observed usage UI, generation/translation provider selection incl. DeepL, provider tiers) run on the real database + pipeline; JSON backup export/restore, CSV import with column mapping, Anki `.apkg` import (v1 scope: no review history, no media, single target deck), manual/clipboard sentence capture, and the full example/synonym evaluation workflow (undo/replace, reports, primary-meaning and flashcard-example selection) are all implemented — share-sheet capture is the one explicitly deferred item. See `PHASE_4_STATUS.md` (tag `v0.4`) for the full deliverable matrix · Phase 5 (flashcards/FSRS) 🟨 `packages/srs` has a real FSRS scheduler (`schedule`, `createInitialCardState`, wraps `ts-fsrs`, `card_states` gained `reps`/`learning_steps` in migration 0006); the review session (real due cards, swipe-gesture + button rating, `recordReview` persistence) and the template editor (Fields/Style/Preview/Code tabs, real LiquidJS + WebView rendering via `apps/mobile/lib/templates.ts`, real `templates` CRUD) are wired end-to-end — only the stats screen is still dummy data. See `PHASE_5_STATUS.md` for the deliverable matrix and work-package plan. `grep -rn "TODO(phase" apps/mobile` lists every pending wiring point. Design docs: `..\Totorials_and_AppsDocs\LingoraDocs\3_phase3_ai_engine_design.md`, `4_phase4_ui_design.md`.
...
There is no root tsconfig; the root `pnpm typecheck` script (`tsc --build`) does not work — typecheck per package/app as above. Vitest is the test runner, scoped to `packages/ai`, `packages/database`, `packages/observability`, and `packages/srs` (tests co-located as `src/**/*.test.ts`; `src/providers/openai.live.test.ts` is an opt-in live smoke that only runs when `OPENAI_API_KEY` is set). Most of the Phase 2 data layer predates Vitest and was verified with a scratch smoke test via `tsx` (see `3_phase2_database_design.md` §11); `packages/database/src/testing/node-sqlite-adapter.ts` is a standalone copy of `packages/ai`'s test adapter (can't import it directly — dependency direction runs the other way).
...
- **Corporate VPN also intercepts HTTPS to external hosts (Zscaler)**: direct `fetch()` calls to AI provider APIs (Mistral, OpenAI, etc.) fail on the AVD with `net::ERR_CERT_AUTHORITY_INVALID` — the host machine trusts the Zscaler root CA, the emulator doesn't. Fixed for debug builds via `apps/mobile/plugins/withDebugUserCaTrust.js` (an Expo config plugin, registered in `app.json`, that writes a debug-only `android/app/src/debug` network security config trusting user-installed CAs). Still requires installing the Zscaler root CA on the AVD once, manually, via Settings → Security & privacy → More security & privacy → Encryption & credentials → Install a certificate → CA certificate → Install anyway (needs a screen lock set first). Never applies to release builds.
...
- **Red "Unable to load script" / blue Expo Go "Something went wrong" screen with Metro confirmed running**: two distinct causes, check both. (1) You're in Expo Go (`host.exp.exponent`) instead of Lingora's own dev client — Expo Go can't load this project (custom native modules/config plugins). Launch the "Lingora" app icon directly, not via Expo Go's project list (`adb shell am start -n com.lingora.mobile/.MainActivity`). (2) Even in the correct dev client, it may still try to reach Metro at the emulator's default `10.0.2.2:8081` alias instead of `localhost:8081` — this corporate network apparently blocks that alias the same way it blocks the LAN IP (see the VPN gotcha above), and `adb reverse tcp:8081 tcp:8081` alone doesn't fix it because the *app* never tries `localhost` in the first place. Neither the Android `Settings.System debug_http_host` key nor re-opening the `lingora://expo-development-client/?url=...` deep link reliably overrides this once the client has cached a host. The fix that actually works: write React Native's own `debug_http_host` preference directly into the app's SharedPreferences file (confirmed against `PackagerConnectionSettings.kt` in `react-native`'s source — this is an app-local preference, not an OS setting, despite the identical key name):
  ```
  adb shell "run-as com.lingora.mobile sh -c 'mkdir -p /data/data/com.lingora.mobile/shared_prefs && cat > /data/data/com.lingora.mobile/shared_prefs/com.lingora.mobile_preferences.xml'" <<'EOF'
  <?xml version='1.0' encoding='utf-8' standalone='yes' ?>
  <map>
      <string name="debug_http_host">localhost:8081</string>
  </map>
  EOF
  adb shell am force-stop com.lingora.mobile && adb shell am start -n com.lingora.mobile/.MainActivity
  ```
  Only needs redoing after a `pm clear`/fresh install, not every launch.
...
- `lib/dummy.ts` now holds only the Phase 5 stats-screen stand-ins (review queue and FSRS interval dummies were removed once the review session was wired to real data); everything else is wired to the database. `settings/import-export.tsx`: JSON backup export/restore is wired to `lib/backup.ts`; the CSV button opens `settings/csv-import.tsx` (over `packages/database/src/csv-import.ts`); the Anki button opens `settings/apkg-import.tsx` (over `packages/database/src/apkg-import.ts`, unzipped in `lib/apkg.ts` via `jszip` + `expo-sqlite#deserializeDatabaseAsync`) — all three are the same pick → map → preview → confirm wizard shape.
