...
/**
 * Navigates by building a real `lemory://` deep link and opening it, rather than calling
 ...
 * same well-tested path a real `lemory://...` link from outside the app already uses — so it's
 ...
 * setting (Settings > General > "Add to Lemory", see `lib/captureIntent.ts`) says it should go,
...
export function CaptureIntentHandler(): JSX.Element | null {
  ...
  //  - A share intent can make Lemory's activity non-task-root, and expo-share-intent's own
