...
/**
 * Opt-in live smoke test against the real Gemini API. Skipped unless GEMINI_API_KEY is set —
 * see packages/ai/.env.example for how to set it locally without ever committing it.
 */
const apiKey = process.env['GEMINI_API_KEY']

describe.skipIf(!apiKey)('GeminiProvider live', () => {
  it(
    'generates a valid word package for ausgehen at B1',
    { timeout: 120_000 },
    async () => {
      const provider = new GeminiProvider({ apiKey: apiKey! })
