...
**Phase status (keep this current):** Phase 1 ✅ · Phase 2 (database/search/morphology) ✅ · Phase 3 (AI) ✅ core engine in `packages/ai` (OpenAI, Mistral, Gemini, and Anthropic/Claude providers — all four implement both `AIProvider` and `DictionaryProvider`; Google Translate free-tier dictionary adapter; validation/repair, prompt versioning, cache, persistence pipeline); DeepL/Wiktionary adapters pending · Phase 4 (DB+AI wiring) 🟨 search, word detail, home, decks, deck detail, mining and settings (SecureStore keys per provider, validate-key + device-observed usage UI, generation/translation provider selection, provider tiers) run on the real database + pipeline; JSON backup export/restore is implemented (`packages/database/src/backup.ts`, `apps/mobile/lib/backup.ts`) — CSV column-mapping and Anki `.apkg` import are still deferred · Phase 5 screens (review session, stats, templates) still dummy — FSRS not built. `grep -rn "TODO(phase" apps/mobile` lists every pending wiring point. Design docs: `..\Totorials_and_AppsDocs\LingoraDocs\3_phase3_ai_engine_design.md`, `4_phase4_ui_design.md`.
